  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-card transition-all duration-300 hover:-translate-y-1 hover:shadow-float">
      <div className="relative aspect-square overflow-hidden bg-secondary">
        {discount > 0 && (
          <span className="absolute left-2 top-2 z-10 rounded-md bg-sunset-gradient px-2 py-0.5 text-[10px] font-bold text-white shadow-coral">
            {discount}% OFF
          </span>
        )}
        <img
      ...
      <div className="flex flex-1 flex-col p-3">
        ...
        <div className="mt-auto flex items-center justify-between gap-2">
          ...
          {inCart ? (
            <div className="flex animate-bounce-in items-center gap-1 rounded-lg bg-primary p-1 shadow-mint">
              <Button
                size="icon"
                variant="ghost"
                onClick={() => remove(product.id)}
                className="h-7 w-7 rounded-md text-primary-foreground hover:bg-white/15 hover:text-primary-foreground"
              >
            ...
            <Button
              onClick={() => add(product)}
              className="h-9 rounded-lg border border-primary/30 bg-primary/10 px-4 text-xs font-bold text-primary shadow-sm transition-all hover:bg-primary hover:text-primary-foreground"
            >
              ADD
            </Button>
          )}
